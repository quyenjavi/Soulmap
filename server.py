...
def _build_endpoint(api_base: str, suffix: str) -> str:
    ...
    if base_no_slash.endswith(suffix):
        return base_no_slash
    return base_no_slash + suffix
            ...
            if not image_url and img_raw:
                if isinstance(img_raw, str):
                    try:
                        ...
                        image_url = (
                            (img_obj.get('images') or [{}])[0].get('url')
                            or (img_obj.get('data') or [{}])[0].get('url')
                            or ''
                        )
                    except Exception:
                        pass
                    ...
                    image_url = (
                        (img_raw.get('images') or [{}])[0].get('url')
                        or (img_raw.get('data') or [{}])[0].get('url')
                        or ''
                    )
